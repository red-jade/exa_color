defmodule Exa.Color.Col4b do
  ...
  """
  @spec new(C.col3b(), I.alpha_value(), I.pixel4()) :: C.col4b()
  ...
  def a1b(a), do: raise(ArgumentError, message: "Illegal alpha value '#{a}'")
  ...
  @spec to_col3b(C.col4b(), I.pixel4()) :: C.col3b()
  ...
  @doc "To RGBA hex."
  @spec to_hex(C.col4b(), I.pixel4()) :: C.hex4()
  ...
  @doc "From RGBA hex."
  @spec from_hex(C.hex4(), I.pixel4()) :: C.col4b()
  ...
  """
  @spec to_css(C.col4b(), I.pixel()) :: String.t()
  ...
  @impl Colorb
  def to_bin(col, pix \\ :rgba), do: append_bin(<<>>, pix, col)

  @impl Colorb
  def append_bin(buf, pix \\ :rgba, col)

  def append_bin(buf, _pix, {c1, c2, c3, c4}) when is_binary(buf),
    do: <<buf::binary, c1, c2, c3, c4>>

  # def append_bin(buf, :rgba, {ir, ig, ib,ia}) when is_binary(buf), do: <<buf::binary, ir, ig, ib, ia>>
  # def append_bin(buf, :argb, {ia,ir, ig, ib}) when is_binary(buf), do: <<buf::binary, ia, ir, ig, ib>>
  # def append_bin(buf, :bgra, {ib, ig, ir,ia}) when is_binary(buf), do: <<buf::binary, ib, ig, ir, ia>>
  # def append_bin(buf, :abgr, {ia,ib, ig, ir}) when is_binary(buf), do: <<buf::binary, ia, ib, ig, ir>>

  @impl Colorb
  def from_bin(buf, pix \\ :rgba)

  def from_bin(<<c1, c2, c3, c4, rest::binary>>, :rgba), do: {{c1, c2, c3, c4}, rest}
  # def from_bin(<<ir, ig, ib, ia, rest::binary>>, :rgba), do: {{ir, ig, ib, ia}, rest}
  # def from_bin(<<ia, ir, ig, ib, rest::binary>>, :argb), do: {{ia, ir, ig, ib}, rest}
  # def from_bin(<<ib, ig, ir, ia, rest::binary>>, :bgra), do: {{ib, ig, ir, ia}, rest}
  # def from_bin(<<ia, ib, ig, ir, rest::binary>>, :abgr), do: {{ia, ib, ig, ir}, rest}
